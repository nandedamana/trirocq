...
Lemma v64_prvcarry_Si (x y : v64) {i} (hidx : S i < SIZE) :
  v64_prvcarry x y hidx = let a := v64_ith x (ltprv hidx) in
                          let b := v64_ith y (ltprv hidx) in
                          let cin := v64_prvcarry x y (ltprv hidx) in
                          bit_or (bit_or (bit_and a b) (bit_and a cin)) (bit_and b cin).
Proof.
...
Axiom v64_fulladd_result2 : forall x y [i] (hidx : i < SIZE), v64_ith (v64_add x y) hidx = bit_xor (v64_prvcarry x y hidx) (bit_xor (v64_ith x hidx) (v64_ith y hidx)).
...
Module tnum.
  ...
  Definition wellformed3 (tn : t) := forall [i] (hidx : i < SIZE), bit_and (v64_ith (m tn) hidx) (v64_ith (v tn) hidx) = zero.

  Definition wellformed2 (tn : t) := v64_and (v tn) (m tn) = zero64.
...
Section linux_tnum_addition.
  ...
  (* TODO rename *)
  Ltac rewrite17 otn := match goal with
                          [ H : ?x = ?x -> otnum.ith otn _ = _ |- _] => rewrite H
                        end.
  ...
  Lemma helper14 v1 v2 {i} (hidx : i < SIZE) :
    v64_ith (v64_xor (v64_add v1 v2) v1) hidx = bit_xor (v64_ith v2 hidx) (v64_prvcarry v1 v2 hidx).
  Proof.
    unwrap_v64_ops.
    rewrite v64_fulladd_result with (x := v1) (y := v2).
    destruct (v64_ith v1 hidx); destruct (v64_ith v2 hidx); destruct (v64_prvcarry v1 v2 hidx); auto.
    auto.
  Qed.
  ...
  (* TODO rename *)
  Lemma helper18 v1 v2 :
    forall i (hidx : S i < SIZE),
      let a := v64_ith v1 (ltprv hidx) in
      let b := v64_ith v2 (ltprv hidx) in
      let c := v64_prvcarry v1 v2 (ltprv hidx) in
      v64_prvcarry v1 v2 hidx = zero ->
      (bit_and a b = zero) /\ (bit_and a c = zero) /\ (bit_and b c = zero).
  Proof.
    intros i hidx.
    rewrite v64_prvcarry_Si.
    destruct (v64_ith v1 (ltprv hidx));
      destruct (v64_ith v2 (ltprv hidx));
      destruct (v64_prvcarry v1 v2 (ltprv hidx));
      unfold bit_and; unfold bit_or; try easy; auto.
  Qed.

  Lemma helper21 (x : bit) : Some x = Some (match x with zero => zero | one => one end).
    destruct x; auto.
  Qed.
  ...
  Lemma helper31 x y P Q :
    tnum.wellformed P -> tnum.wellformed Q -> ingamma x P -> ingamma y Q ->
    forall [i] (hidx : i < SIZE),
      v64_prvcarry (tnum.v P) (tnum.v Q) hidx = one -> v64_prvcarry x y hidx = one.
  Proof.
    unfold tnum.wellformed. unfold ingamma. unfold tnum.ith_m.
    intros wf1 wf2 igP igQ.
    unfold ingamma.
    ...
    induction i.
    -
      unfold v64_prvcarry. auto.
    -
      (* TODO why don't do early? *)
      specialize (wf1 i (ltprv hidx)).
      specialize (wf2 i (ltprv hidx)).
      specialize (igP i (ltprv hidx)).
      specialize (igQ i (ltprv hidx)).
      specialize (IHi (ltprv hidx)).
      
      repeat rewrite v64_prvcarry_Si.
      destruct (v64_ith (tnum.m P) (ltprv hidx));
        destruct (v64_ith (tnum.m Q) (ltprv hidx));
        try rewrite_if_holds wf1; try rewrite_if_holds wf2; auto;
        try rewrite_if_holds igP; try rewrite_if_holds igQ; auto;
        unfold tnum.ith_v;
        destruct (v64_prvcarry (tnum.v P) (tnum.v Q) (ltprv hidx));
        ...
        destruct (v64_prvcarry x y (ltprv hidx)); simpl; try easy;
        destruct (bit_xor (v64_ith x (ltprv hidx)) (v64_ith y (ltprv hidx))); try easy; simpl.
  Qed.
  ...
  Lemma helper32 P Q :
    ...
    forall [i] (hidx : i < SIZE),
      ...
      tnum.ith_m P hidx = zero /\ tnum.ith_m Q hidx = zero /\ (* TODO rem? *)
        tnum_ith_chi P Q hidx = zero.
  Proof.
    unfold tnum.wellformed. unfold ingamma. unfold tnum_ith_chi.
    intros wf1 wf2.
    (* TODO remove anything useless from above *)

    destruct i. (* TODO do without destructing *)
    - intro hidx. (* TODO do early? *)
      unfold tnum_add.
      rewrite tnum.ith_m_simplify.
      unwrap_v64_ops. unfold tnum.ith_m.
      rewrite v64_fulladd_result2.
      unfold v64_prvcarry.

      destruct (bit_xor
                  (bit_xor zero
                     (bit_xor (v64_ith (v64_add (tnum.v P) (tnum.v Q)) hidx)
                        (v64_ith (v64_add (tnum.m P) (tnum.m Q)) hidx)))
                  (v64_ith (v64_add (tnum.v P) (tnum.v Q)) hidx));
      
        destruct (v64_ith (tnum.m P) hidx);
        destruct (v64_ith (tnum.m Q) hidx); try easy; auto.
    - intro hidx. (* TODO do early? *)
      unfold tnum_add.
      rewrite tnum.ith_m_simplify.
      unwrap_v64_ops. unfold tnum.ith_m.
      ...
      destruct (bit_xor
                  (bit_xor
                     (v64_prvcarry (v64_add (tnum.v P) (tnum.v Q))
                        (v64_add (tnum.m P) (tnum.m Q)) hidx)
                     (bit_xor (v64_ith (v64_add (tnum.v P) (tnum.v Q)) hidx)
                        (v64_ith (v64_add (tnum.m P) (tnum.m Q)) hidx)))
                  (v64_ith (v64_add (tnum.v P) (tnum.v Q)) hidx));
  ...
  Lemma helper40 [x] [y] : bit_xor x y = zero -> x = y.
    unfold bit_xor. destruct x; destruct y; auto.
  Qed.

  Lemma helper42 x y : bit_xor x y = zero -> x = y.
    unfold bit_xor. destruct x; destruct y; auto.
  Qed.
  
  Lemma helper41 sv sm :
    forall [i] (hidx : i < SIZE),
      let sig := v64_add sv sm in
      let chi := v64_xor sig sv in
      v64_ith chi hidx = zero ->
      v64_ith (v64_add sv sm) hidx = v64_ith sv hidx.
  Proof.
    intros until hidx.
    simpl. rewrite v64_ith_unwrap_xor.
    apply helper42.
  Qed.

  Lemma helper43 sv sm :
    forall [i] (hidx : i < SIZE),
      v64_ith (v64_add sv sm) hidx = v64_ith sv hidx ->
      bit_xor (v64_prvcarry sv sm hidx) (v64_ith sm hidx) = zero.
  Proof.
    intros until hidx.
    rewrite v64_fulladd_result2.
    destruct (v64_prvcarry sv sm hidx);
      destruct (v64_ith sv hidx);
      destruct (v64_ith sm hidx); auto.
  Qed.

  Lemma helper44 sv sm :
    forall [i] (hidx : i < SIZE),
      bit_xor (v64_prvcarry sv sm hidx) (v64_ith sm hidx) = zero ->
      v64_ith sm hidx = zero -> v64_prvcarry sv sm hidx = zero.
  Proof.
    intros until hidx.
    destruct (v64_ith sm hidx).
    - rewrite bit_xor_right_zero. auto.
    - easy.
  Qed.

  (* TODO wait, helper42-helper44? - what helper14 does already *)

  Lemma helper46 P Q :
    tnum.wellformed P -> tnum.wellformed Q ->
    forall [i] (hidx : i < SIZE),
      v64_prvcarry (tnum.m P) (tnum.m Q) hidx = one ->
      v64_prvcarry (tnum.v P) (tnum.v Q) hidx = zero.
  Proof.
    unfold tnum.wellformed.
    intros wfp wfq i hidx.
    induction i.
    - unfold v64_prvcarry. trivial.
    - rewrite v64_prvcarry_Si. simpl.

      specialize (wfp i (ltprv hidx)).
      specialize (wfq i (ltprv hidx)).
      specialize (IHi (ltprv hidx)).

      destruct (v64_prvcarry (tnum.m P) (tnum.m Q) (ltprv hidx));
        try rewrite_if_holds IHi;
        destruct (v64_ith (tnum.m P) (ltprv hidx));
          destruct (v64_ith (tnum.m Q) (ltprv hidx));
  ...
  Lemma helper66 P Q :
    tnum.wellformed P -> tnum.wellformed Q ->
    forall [i] (hidx : i < SIZE),
      bit_and (v64_prvcarry (tnum.m P) (tnum.m Q) hidx)
        (v64_prvcarry (tnum.v P) (tnum.v Q) hidx) = zero.
  Proof.
    ...
    - unfold v64_prvcarry. auto.
    - intros hidx.
      repeat rewrite v64_prvcarry_Si. simpl.
      repeat rewrite v64_fulladd_result2.
  ...
  Lemma helper63 P Q :
    ...
    forall [i] (hidx : i < SIZE),
      bit_and (v64_prvcarry (tnum.m P) (tnum.m Q) hidx)
        ...
        bit_and (v64_prvcarry (tnum.v P) (tnum.v Q) hidx)
          (v64_prvcarry (v64_add (tnum.v P) (tnum.v Q))
             (v64_add (tnum.m P) (tnum.m Q)) hidx) = zero.
  Proof.
    unfold tnum.wellformed. unfold ingamma.
    ...
    - intros hidx.
      ...
      assert (h66 : bit_and (v64_prvcarry (tnum.m P) (tnum.m Q) (ltprv hidx))
                      (v64_prvcarry (tnum.v P) (tnum.v Q) (ltprv hidx)) = zero).
      apply helper66; auto.
      ...
      destruct (v64_prvcarry (tnum.v P) (tnum.v Q) (ltprv hidx));
      destruct (v64_prvcarry (v64_add (tnum.v P) (tnum.v Q))
  ...
  Lemma helper49 P Q :
    tnum.wellformed P -> tnum.wellformed Q ->
    forall [i] (hidx : i < SIZE),
      v64_prvcarry (tnum.m P) (tnum.m Q) hidx = zero ->
      let sv := v64_add (tnum.v P) (tnum.v Q) in
      let sm := v64_add (tnum.m P) (tnum.m Q) in
      v64_prvcarry sv sm hidx = one ->
      v64_prvcarry (tnum.v P) (tnum.v Q) hidx = zero.
  Proof.
    unfold tnum.wellformed.
    intros wfp wfq i hidx.
    ...
    - rewrite v64_prvcarry_Si. simpl.
      ...
      assert (h64 : bit_and (v64_prvcarry (tnum.m P) (tnum.m Q) (ltprv hidx))
                      (v64_prvcarry (v64_add (tnum.v P) (tnum.v Q))
                         (v64_add (tnum.m P) (tnum.m Q)) (ltprv hidx)) = zero /\
                      bit_and (v64_prvcarry (tnum.v P) (tnum.v Q) (ltprv hidx))
                        (v64_prvcarry (v64_add (tnum.v P) (tnum.v Q))
                           (v64_add (tnum.m P) (tnum.m Q)) (ltprv hidx)) = zero).
      apply helper63; auto.
  ...
  Lemma helper45 x y P Q :
    ...
    forall [i] (hidx : S i < SIZE),
      ...
      let sv := v64_add (tnum.v P) (tnum.v Q) in
      let sm := v64_add (tnum.m P) (tnum.m Q) in
      v64_prvcarry sv sm hidx = zero ->
      v64_prvcarry x y hidx = v64_prvcarry (tnum.v P) (tnum.v Q) hidx.
  Proof.
    unfold tnum.wellformed.
    unfold ingamma.
    ...
    - intro hidx.
      rewrite v64_prvcarry_Si.
      rewrite v64_prvcarry_Si with (x := x).
      repeat rewrite v64_prvcarry_Si with (hidx := hidx).
      repeat rewrite v64_fulladd_result2.
  ...
  Proof.
    ...
    -
      ...
      unfold tnum_add.
      rewrite tnum.ith_m_simplify.
      unwrap_v64_ops. repeat rewrite v64_fulladd_result2.
  ...
  (* TODO include tnum_add_wellformed *)
  ...
  Proof.
    ...
    pose (H' := helper31 x y P Q wf1 wf2 igP igQ).
    
    pose (H'' := helper32 P Q wf1 wf2). unfold tnum.ith_m in H''. (* TODO rename *)
    unfold ingamma.

    intros i hidx rmskz.

    specialize (H' i hidx). (* TODO useless? *)
    specialize (H'' i hidx rmskz) as (xmskz & ymskz & cinmskz).
    ...
    pose (igP' := igP i hidx xmskz).
    pose (igQ' := igQ i hidx ymskz).
    ...
    (* rewrite helper14. (* TODO helper14 is ultimately useless? *) *)
    unwrap_v64_ops. repeat rewrite v64_fulladd_result2.
  ...
  Lemma helper50 v m : tnum.v (tnum.cons v m) = v.
    auto.
  Qed.

  Lemma helper51 v m : tnum.m (tnum.cons v m) = m.
    auto.
  Qed.
